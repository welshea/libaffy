...
#endif
#ifndef NTH
# define NTH(fct) fct THROW
#endif

#define argp_base_name(arg) arg

#if defined HAVE_DECL_PROGRAM_INVOCATION_SHORT_NAME
# define argp_short_program_name()    (program_invocation_short_name)
#else
extern char *argp_short_program_name (void);
#endif
...
#ifndef __restrict
# if !(defined(__GNUC__) && (2 < __GNUC__ || (2 == __GNUC__ && 95 <= __GNUC_MINOR__)))
...
error_t argp_parse (const struct argp *__restrict argp,
			   ...
			   unsigned __flags, int *__restrict arg_index,
...
extern void (*argp_program_version_hook) (FILE *__restrict stream,
...
void argp_state_help (const struct argp_state *__restrict state,
			     FILE *__restrict stream,
			     unsigned int flags);
...
void argp_failure (const struct argp_state *__restrict __state,
...
void *argp_input (const struct argp *__restrict argp,
			  const struct argp_state *__restrict state)
     THROW;

...
ARGP_EI int
option_is_short (const struct argp_option *opt)
{
  if (opt->flags & OPTION_DOC)
    ...
    {
      int key = opt->key;
      return key > 0 && key <= UCHAR_MAX && isprint (key);
    }
}
