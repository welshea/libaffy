...
#define oend(opt) option_is_end (opt)

/* Returns true if OPT has a short option.  */
#define oshort(opt) option_is_short (opt)

/*
	       ...
	       const struct hol_entry *entry2)
{
  ...
  if (entry1->cluster != entry2->cluster)
	...
	/* Only long options.  */
	return (rc = strcasecmp (long1, long2)) ?
                 rc : HOL_ENTRY_PTRCMP(entry1, entry2);
      else
	...
	if (opt->name && *opt->name && ovisible (opt))
	  {
	    ...
	    argp_fmtstream_puts (stream,opt->name);
	  }
    }
  else
    /* A real long option.  */
       ...
       unsigned flags, char *name)
{
  ...
  if (flags & (ARGP_HELP_USAGE | ARGP_HELP_SHORT_USAGE))
      ...
      do
	{
	  ...
	  if (first_pattern)
	    argp_fmtstream_printf (fs, "Usage: %s", name);
	  else
	    argp_fmtstream_printf (fs, "  or: %s", name);
...
#ifdef weak_alias
weak_alias (argp_state_help, argp_state_help)
#endif

...
argp_error (const struct argp_state *state, const char *fmt, ...)
{
  if (!state || !(state->flags & ARGP_NO_ERRS))
      ...
      if (stream)
	{
	  va_list ap;
	  va_start (ap, fmt);
	  fputs(state ? state->name : argp_short_program_name (), stream);
          putc(':', stream);
	  putc(' ', stream);
...
/*	      char buf[200]; */
		{
		  ...
		  putc(':', stream);
		  putc(' ', stream);
/* Commented out for portability reasons. */
